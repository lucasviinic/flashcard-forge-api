...
@router.put("/{user_id}")
async def update_user(user: user_dependency, db: db_dependency, user_request: UserRequest, user_id: str):
    if not user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='authentication failed')

    try:
        user_data = update_user_usecase(db, user_id=user_id, user_request=user_request)
    except Exception as e:
